...
Channel-entry coverage includes no-photo, single-photo, media-group, maximum-photo validation, button-style mapping and fallback, correct entry-message pinning, upload validation, authorization, compatibility defaults, and publication-history fields.
...
- channel entry with zero, one, and multiple photos, including pinning and styled-button fallback
