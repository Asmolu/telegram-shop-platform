...
def redact_secret(value: str) -> str:
    secret = settings.telegram_seller_webhook_secret
    if not secret:
        return value
...
def _required_webhook_secret() -> str:
    secret = settings.telegram_seller_webhook_secret
    if not secret:
        raise TelegramDeliveryError("TELEGRAM_SELLER_WEBHOOK_SECRET is not configured")
    return secret


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Manage the Seller Bot 2 Telegram webhook.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    set_parser = subparsers.add_parser("set", help="Set the Seller Bot 2 webhook")
    ...
    subparsers.add_parser("info", help="Show redacted Seller Bot 2 webhook info")
